...
#Atenção: Mude o nome da base de dados para "BD_seunome"
create database BD_MarcoRigon;
use BD_MarcoRigon;
...
#Exercício 1
select
Conta_Corrente.cod_cc as 'Código',
Conta_Corrente.numero_cc as 'Número',
...
#Pegar dados da conta de origem
conta_corrente.numero_cc as 'Número_Conta_Origem',
conta_corrente.saldo_cc as 'Saldo_Conta_Origem',
cliente.nome_cli as 'Nome_Cliente_Origem',
#Pegar dados da conta de destino
(select
conta_corrente.numero_cc from conta_corrente where (transferencia.cod_cc_destino_fk = conta_corrente.cod_cc)
) as 'Número_Conta_Destino',
(select
conta_corrente.saldo_cc from conta_corrente where (transferencia.cod_cc_destino_fk = conta_corrente.cod_cc)
) as 'Saldo_Conta_Destino',
(select
cliente.nome_cli from cliente where (transferencia.cod_cc_destino_fk = cliente.cod_cli)
) as 'Nome_Cliente_Destino'

from
transferencia, conta_corrente, cliente
where
(cliente.cod_cli =conta_corrente.cod_cli_fk) and
(conta_corrente.cod_cc = transferencia.cod_cc_origem_fk);



#Exercício 3
#Selecione todos os registros do Saque substituindo as FK de Conta Corrente por número e saldo da Conta, e mostre também o nome do Cliente.
select
saque.cod_saq as 'Código_Saque',
saque.valor_saq as 'Valor_Saque',
saque.data_saq as 'Data_Saque',
saque.hora_saq as 'Hora_Saque',
saque.local_saq as 'Local_Saque',
conta_corrente.numero_cc as 'Número_Conta',
conta_corrente.saldo_cc as 'Saldo',
cliente.nome_cli as 'Saque_feito_por'
from
...
#Exercício 4
#Selecione todos os registros do Depósito substituindo as FK de Conta Corrente por número e saldo da Conta, e mostre também o nome do Cliente.
select
deposito.cod_dep as 'Código',
deposito.valor_dep as 'Valor_Depósito',
deposito.data_dep as 'Data_Depósito',
deposito.tipo_dep as 'Tipo_Depósito',
conta_corrente.numero_cc as 'Número_Conta',
conta_corrente.saldo_cc as 'Saldo_Conta',
cliente.nome_cli as 'Nome_Cliente'
from
...
#Exercício 5
#Selecione todos os registros do Pagamento substituindo as FK de Conta Corrente por número e saldo da Conta, e mostre também o nome do Cliente.
select
pagamento.cod_pag as 'Código',
pagamento.valor_pag as 'Valor',
pagamento.data_pag as 'Data_pagamento',
pagamento.tipo_pag as 'Tipo_pagamento',
pagamento.dataVencimento_pag as 'Data_Vencimento',
pagamento.codigoBarras_pag as 'Código_Barras',
conta_corrente.numero_cc as 'Número_Conta',
conta_corrente.saldo_cc as 'Saldo_Conta',
cliente.nome_cli as 'Nome_Cliente'
from
...
#Exercício 6
#Selecione o código e nome do Cliente, informando o nome do seu Banco. 
#cliente > conta_corrente > agencia > Banco
select
cliente.cod_cli as 'Código_Cliente',
cliente.nome_cli as 'Nome_Cliente',
banco.nome_ban as 'Banco_Cliente'
from
cliente, banco, conta_corrente, agencia
where
...
#Exercício 7
#Selecione o nome do Banco, o número da Agência, o nome do Cliente, o número e o saldo da sua Conta Corrente.
select
banco.nome_ban as 'Nome_Banco',
agencia.numero_ag as 'Número_Agência',
cliente.nome_cli as 'Nome_Cliente',
conta_corrente.numero_cc as 'Número_conta',
conta_corrente.saldo_cc as 'Saldo_conta'
from
banco, agencia, conta_corrente, cliente
where
(conta_corrente.cod_cli_fk = cliente.cod_cli) and
(conta_corrente.cod_ag_fk = agencia.cod_ag) and
(agencia.cod_ban_fk = banco.cod_ban);
...
#Exercício 8
#Selecione o nome do Cliente, número da Conta Corrente, a soma, a média, o valor máximo, o valor mínimo e a quantidade de Saques realizados.
select
cliente.nome_cli as 'Nome_do_Cliente',
conta_corrente.numero_cc as 'Número_da_conta',
(select SUM(valor_saq) from saque where (conta_corrente.cod_cc = saque.cod_cc_fk)) as 'Quantia_total_sacada',
(select AVG(valor_saq) from saque where (conta_corrente.cod_cc = saque.cod_cc_fk)) as 'Média_saques',
(select MAX(valor_saq) from saque where (conta_corrente.cod_cc = saque.cod_cc_fk)) as 'Maior_saque',
(select MIN(valor_saq) from saque where (conta_corrente.cod_cc = saque.cod_cc_fk)) as 'Menor_saque',
(select COUNT(valor_saq) from saque where (conta_corrente.cod_cc = saque.cod_cc_fk)) as 'Quantidade_saques'
from
cliente inner join conta_corrente on (conta_corrente.cod_cli_fk = cliente.cod_cli);



#Exercício 9
#Selecione o nome do Cliente, número da Conta Corrente, a soma, a média, o valor máximo, o valor mínimo e a quantidade de Depósitos realizados. 
select
cliente.nome_cli as 'Nome_do_Cliente',
conta_corrente.numero_cc as 'Número_da_conta',
(select SUM(valor_dep) from deposito where (conta_corrente.cod_cc = deposito.cod_cc_fk)) as 'Quantia_total_depositada',
(select AVG(valor_dep) from deposito where (conta_corrente.cod_cc = deposito.cod_cc_fk)) as 'Média_depositos',
(select MAX(valor_dep) from deposito where (conta_corrente.cod_cc = deposito.cod_cc_fk)) as 'Maior_deposito',
(select MIN(valor_dep) from deposito where (conta_corrente.cod_cc = deposito.cod_cc_fk)) as 'Menor_deposito',
(select COUNT(valor_dep) from deposito where (conta_corrente.cod_cc = deposito.cod_cc_fk)) as 'Quantidade_depositos'
from
cliente inner join conta_corrente on (conta_corrente.cod_cli_fk = cliente.cod_cli);
...
#Exercício 10
#Selecione o nome do Cliente, número da Conta Corrente, a soma, a média, o valor máximo, o valor mínimo e a quantidade de Pagamentos realizados. 
select
cliente.nome_cli as 'Nome_do_Cliente',
conta_corrente.numero_cc as 'Número_da_conta',
(select SUM(valor_pag) from pagamento where (conta_corrente.cod_cc = pagamento.cod_cc_fk)) as 'Quantia_total_paga',
(select AVG(valor_pag) from pagamento where (conta_corrente.cod_cc = pagamento.cod_cc_fk)) as 'Média_pagamentos',
(select MAX(valor_pag) from pagamento where (conta_corrente.cod_cc = pagamento.cod_cc_fk)) as 'Maior_pagamento',
(select MIN(valor_pag) from pagamento where (conta_corrente.cod_cc = pagamento.cod_cc_fk)) as 'Menor_pagamento',
(select COUNT(valor_pag) from pagamento where (conta_corrente.cod_cc = pagamento.cod_cc_fk)) as 'Quantidade_pagamentos'
from
...
#Exercício 11
#Armazene todas as 10 consultas anteriores em VIEWS.
#Parenteses para tela menos caótica B^)
...
#view 2
create view dados_transferencia as
(select
transferencia.cod_trans as 'Código',
transferencia.valor_trans as 'Valor',
transferencia.data_trans as 'Data',
...
#view 8
create view informação_saque as
(select
cliente.nome_cli as 'Nome_do_Cliente',
conta_corrente.numero_cc as 'Número_da_conta',
(select SUM(valor_saq) from saque where (conta_corrente.cod_cc = saque.cod_cc_fk)) as 'Quantia_total_sacada',
...
#view 9
create view informação_deposito as
(select
cliente.nome_cli as 'Nome_do_Cliente',
conta_corrente.numero_cc as 'Número_da_conta',
(select SUM(valor_dep) from deposito where (conta_corrente.cod_cc = deposito.cod_cc_fk)) as 'Quantia_total_depositada',
...
#view 10
create view informação_pagamento as
(select
cliente.nome_cli as 'Nome_do_Cliente',
conta_corrente.numero_cc as 'Número_da_conta',
(select SUM(valor_pag) from pagamento where (conta_corrente.cod_cc = pagamento.cod_cc_fk)) as 'Quantia_total_paga',
...
#Exercício 12
#Faça 01 consulta personalizada em cada VIEW criada.

select * from dados_conta_corrente where (Numero_Agencia = 1824);

select * from dados_transferencia where (Número_Conta_Destino = 40650);

select Data_Saque, Hora_Saque, Saque_feito_por from dados_saque where (Local_Saque = 'Caixa Eletrônico');

select Data_Depósito, Valor_Depósito from dados_deposito where (Número_Conta = 280541);

select Data_Vencimento, Valor, Número_Conta, Nome_Cliente from dados_pagamento order by Data_Vencimento;

select Nome_Cliente from banco_cliente where (Banco_Cliente = 'Banco do Brasil');

select Número_Conta, Nome_Cliente from dados_cliente_banco where (Número_Agência = '0951-2');

select Nome_do_Cliente, Número_da_conta, Quantidade_saques from informação_saque where (Quantidade_saques > 0);

select Nome_do_Cliente, Número_da_conta, Quantidade_depositos from informação_deposito where (Quantidade_depositos > 0);

select Nome_do_Cliente, Número_da_conta, Quantidade_pagamentos from informação_pagamento where (Quantidade_pagamentos > 0);
