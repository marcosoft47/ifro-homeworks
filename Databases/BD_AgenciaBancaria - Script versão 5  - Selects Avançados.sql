...
#Exercício 1
describe conta_corrente;
select
Conta_Corrente.cod_cc as 'Código',
Conta_Corrente.numero_cc as 'Número',
Conta_Corrente.dataAbertura_cc as 'Data_de_Abertura',
Conta_Corrente.saldo_cc as 'Saldo',
Conta_Corrente.valorLimite_cc as 'Valor_do_Limite',
Conta_Corrente.saldoComLimite_cc as 'Saldo_com_Limite',
Agencia.numero_ag as 'Nº_Agencia',
...
(select
conta_corrente.numero_cc from conta_corrente where (transferencia.cod_cc_origem_fk = conta_corrente.cod_cc)
) as 'Número Conta Origem',
(select
conta_corrente.saldo_cc from conta_corrente where (transferencia.cod_cc_origem_fk = conta_corrente.cod_cc)
) as 'Saldo Conta Origem',
(select
cliente.nome_cli from cliente where (transferencia.cod_cc_origem_fk = conta_corrente.cod_cc) and (conta_corrente.cod_cli_fk = cliente.cod_cli)
) as 'Nome Cliente Origem',
...
) as 'Número Conta Destino',
(select
conta_corrente.saldo_cc from conta_corrente where (transferencia.cod_cc_destino_fk = conta_corrente.cod_cc)
) as 'Saldo Conta Destino',
(select
cliente.nome_cli from cliente where (transferencia.cod_cc_destino_fk = conta_corrente.cod_cc) and (conta_corrente.cod_cli_fk = cliente.cod_cli)
) as 'Nome Cliente Destino'

from
transferencia, conta_corrente, cliente
where
(cliente.cod_cli =conta_corrente.cod_cli_fk) and
(conta_corrente.cod_cc = transferencia.cod_cc_origem_fk) and
(conta_corrente.cod_cc = transferencia.cod_cc_destino_fk);
...
select* from saque;
select
saque.cod_saq as 'Código_do_Saque',
saque.valor_saq as 'Valor_do_Saque',
saque.data_saq as 'Data_do_Saque',
saque.hora_saq as 'Hora_do_Saque',
saque.local_saq as 'Local_de_Saque',
...
deposito.tipo_dep as 'Tipo_do_Depósito',
conta_corrente.numero_cc as 'Número_da_Conta',
conta_corrente.saldo_cc as 'Saldo_da_Conta',
cliente.nome_cli as 'Nome_do_Cliente'
from
...
pagamento.data_pag as 'Data_do_pagamento',
pagamento.tipo_pag as 'Tipo_de_pagamento',
pagamento.dataVencimento_pag as 'Data_de_Vencimento',
pagamento.codigoBarras_pag as 'Código_de_Barras',
conta_corrente.numero_cc as 'Número_da_Conta',
conta_corrente.saldo_cc as 'Saldo_da_Conta',
cliente.nome_cli as 'Nome_do_Cliente'
from
...
select * from conta_corrente;
select
cliente.cod_cli as 'Código_do_Cliente',
cliente.nome_cli as 'Nome_do_Cliente',
banco.nome_ban as 'Banco_do_Cliente'
from
...
select
banco.nome_ban as 'Nome_do_Banco',
agencia.numero_ag as 'Número_da_Agência',
cliente.nome_cli as 'Nome_do_Cliente',
conta_corrente.numero_cc as 'Número_da_conta',
conta_corrente.saldo_cc as 'Saldo_da_conta'
from
...
(select AVG(valor_saq) from saque where (conta_corrente.cod_cc = saque.cod_cc_fk)) as 'Média_de_saques',
...
(select COUNT(valor_saq) from saque where (conta_corrente.cod_cc = saque.cod_cc_fk)) as 'Quantidade_de_saques'
from
...
(select AVG(valor_dep) from deposito where (conta_corrente.cod_cc = deposito.cod_cc_fk)) as 'Média_de_depositos',
...
(select COUNT(valor_dep) from deposito where (conta_corrente.cod_cc = deposito.cod_cc_fk)) as 'Quantidade_de_depositos'
from
cliente inner join conta_corrente on (conta_corrente.cod_cli_fk = cliente.cod_cli);
...
(select AVG(valor_pag) from pagamento where (conta_corrente.cod_cc = pagamento.cod_cc_fk)) as 'Média_de_pagamentos',
...
(select COUNT(valor_pag) from pagamento where (conta_corrente.cod_cc = pagamento.cod_cc_fk)) as 'Quantidade_de_pagamentos'
from
cliente inner join conta_corrente on (conta_corrente.cod_cli_fk = cliente.cod_cli);